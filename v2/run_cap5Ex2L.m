...
m = 15; % Projection dimension
n = 2; % system size
R = 1; % radius of  Newton-Kantorovik verification
intvSol = [-1, 1]; % interval [t0, L], L > t0, of solution domain.
initCond = {-1/sqrt(10+1),10/(10+1)^(3/2)};% u(t0) = initCond. 
finalCond = {1/sqrt(10+1),10/(10+1)^(3/2)};% For IVP finalCond=[] 

%% Define the equation and its derivatives 
f = @(t,x) [x(2),-3*(10)*x(1)/(10+t^2)^2];
d_uf = @(t,x)[0, 1; -3*(10)/(10+t^2)^2, 0];
...
A = zeros(length(x), n);
% true solution
u = @(t) [t/sqrt(10+t^2),10/(10+t^2)^(3/2)];
B = zeros(length(x), n);
for i=1:length(x) 
    A(i,:) = w(x(i));
    B(i,:) = u(x(i));
