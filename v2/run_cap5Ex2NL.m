...
initCond = {0.63678,[]}; % initCond = {u1(t0), u2(t0),...,un(t0)}, put [] on emty spaces for BVP's
finalCond = {[],-0.66}; % For IVP finalCond={}. For BVP finalCond = {u1(L), u2(L),...,un(L)}, put [] on emty spaces  

%% Define the equation, its derivatives and an initial guess
f = @(t,x) [x(2),x(2)-2*x(1)^2]; % reactor's equation
d_uf = @(t,x)[0, 1; -4*x(1), 1];
...
%legend({'ODE45 initial guess', 'Initial Guess fixed to New Base','bvp4c numerical solution','bvp4c numerical solution fixed to New Base','Função coordenada w_2'},'Location','northwest')
xlabel('Time t')
%xlabel('Tempo t')
