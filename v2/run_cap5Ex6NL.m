...
intvSol = [0.1,1]; % interval [t0, L], L > t0, of solution domain.
initCond = {[],0,[],0}; % initCond = {u1(t0), u2(t0),...,un(t0)}, put [] on emty spaces for BVP's
finalCond = {[],0.85,[],0.85}; % For IVP finalCond={}. For BVP finalCond = {u1(L), u2(L),...,un(L)}, put [] on emty spaces  
k = 0.95; % Ginzburg-Landau's parameters
d = 1; % Ginzburg-Landau's parameters

%% Define the equation, its derivatives 
f = @(t,x) d.*[x(2),(k^2)*x(1)*((x(1)^2)+(x(3)^2)-1),x(4),(x(1)^2)*x(3)]; % Ginzburg-Landau's equation  
d_uf = @(t,x) d.*[0,1,0,0;2*(k^2)*x(1)*((x(1)^2)+(x(3)^2)-1),0,2*(k^2)*x(1)*x(3),0;0,0,0,1;2*x(1)*x(3),0,x(1)^2,0];
